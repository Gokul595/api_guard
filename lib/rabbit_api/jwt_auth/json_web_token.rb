module JwtAuth
  ...
  module JsonWebToken
    ...
    def create_token_and_set_header
      ...
      @user.update_attributes(token_issued_at: Time.at(token_issued_at).utc)
    ...
    def destroy_and_blacklist_token
      ...
      @user.blacklisted_tokens.create(token: @token, expire_at: Time.at(@decoded_token[:exp]).utc)
    end
    ...
    def set_token_headers(token, refresh_token = nil)
      ...
      response.headers['Expire-At'] = token_expire_at
    end
  end
end
